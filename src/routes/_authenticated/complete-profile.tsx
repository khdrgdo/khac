...
function CompleteProfilePage() {
  ...
  useEffect(() => {
    if (profile) {
      ...
      setName(profile.full_name && profile.full_name !== "مستخدم" ? profile.full_name : (user?.user_metadata?.full_name ?? ""));
  ...
  async function save() {
    if (!user) return;
    if (univ.trim().length < 6) { toast.error("الرقم الجامعي قصير"); return; }
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <div className="space-y-1.5">
            ...
            <Input value={univ} onChange={(e) => setUniv(e.target.value)} dir="ltr" placeholder="202312345" />
          </div>
          <div className="space-y-1.5">
